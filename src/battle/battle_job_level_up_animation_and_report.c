...
s32 battle_job_level_up_animation_and_report(battle_misc_data_t* misc) {
    if (misc->job_level_for_display != 0) {
        u8 battle_id;
        battle_set_message_duration_frames(0xB4);
        battle_id = misc->battle_data->misc_unit_id;
        system_function_setup(0xA, 0x183E, battle_id, battle_id, 1);
