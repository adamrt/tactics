...
void world_blue_remove_unit(s32 unit_id) {
    s32 resolved_id;
    s32 unit_index;
    s32 misc_id;
    battle_stats_t* unit;
    ...
    do {
        if (battle_does_misc_unit_id_exist(misc_id) != 0) {
            ...
            if (unit_index != -1) {
                unit = find_unit_by_id(get_battle_stats_pointer(unit_index)->unit_id,
                    &resolved_id);
                if ((unit->team_flags & 0x30) != 0 && resolved_id != -2 && unit->unit_id == unit_id) {
