...
typedef struct RequireTexturePage {
    s16 x;
    u16 y;
} RequireTexturePage;

typedef struct RequireStatusRenderStack {
    s32 first_status;
    u8 padding[20];
    const u16* group_y_offset;
} RequireStatusRenderStack;

extern u8 g_require_status_group_count;
extern u8 g_require_status_group_ids[];
extern u8 g_require_status_group_bounds[];
extern u8 g_require_status_animation[];
extern const u16 g_require_status_y_offsets[];
extern u16 g_require_status_position_y;
extern const RequireTexturePage g_require_status_texture;
extern const s16 g_require_status_uv_rects[][6];
extern const s16 g_require_status_scales[][2];
extern const s16 g_require_status_offsets[][6];

extern void battle_make_default_poly_ft4(POLY_FT4* poly);
extern void require_configure_scaled_poly_ft4(
    u8* poly, const RequireTexturePage* texture, const s16* position,
...
void require_render_transition_poly_ft4_passes(POLY_FT4* poly) {
    ...
    volatile RequireStatusRenderStack stack;
    ...
    register const RequireTexturePage* texture __asm__("$5");
    register const s16* scale __asm__("$2");
    s32 initial_group_count = g_require_status_group_count;
    s32 continue_groups;

    if (initial_group_count > 0) {
        position_y = &g_require_status_position_y;
        animation = g_require_status_animation;
        offset_index = 0;
        __asm__("" : "=r"(offset_index) : "0"(offset_index));
        one = (s32)g_require_status_y_offsets;
        stack.group_y_offset = (const u16*)one;
        do {
            group_id = g_require_status_group_ids[group_index];
            last_status = g_require_status_group_bounds[group_id + 1];
            start_status = g_require_status_group_bounds[group_id];
            ...
            if (status_count > 0) {
                ...
                do {
                    ...
                    texture = &g_require_status_texture;
                    __asm__("" : : "r"(geometry_poly), "r"(texture));
                    temp_v1 = (u32)&g_require_status_scales[0][0];
                     ...
                     *   require_configure_scaled_poly_ft4(poly, &texture, position_y - 1,
                    ...
                    __asm__ volatile(
                        "sw $2,0x10($29)\n"
                        "lui $2,%%hi(g_require_status_offsets)\n"
                        "addiu $2,$2,%%lo(g_require_status_offsets)\n"
                        "lhu $3,0($22)\n"
                        "addu $2,$30,$2\n"
                        "sw $2,0x14($29)\n"
                        "lui $2,%%hi(g_require_status_uv_rects)\n"
                        "addiu $2,$2,%%lo(g_require_status_uv_rects)\n"
                        "addu $3,$3,$7\n"
                        "sll $7,$6,1\n"
                        "addu $7,$7,$6\n"
                        "sll $7,$7,2\n"
                        ...
                        "\t.set\tnoreorder\n"
                        "\t.set\tnomacro\n"
                        "\tjal require_configure_scaled_poly_ft4\n"
                        "\tsh $3,0($22)\n"
                        "\t.set\tmacro\n"
                        "\t.set\treorder\n"
            ...
            one += 4;
            continue_groups = group_index < g_require_status_group_count;
            ((RequireStatusRenderStack*)&stack)->group_y_offset = (const u16*)one;
