...
extern s32 g_debugchr_indicator_packet_index[2];
extern s32 g_debugchr_indicator_brightness[2];
extern s32 g_debugchr_indicator_fade_work[2];
extern s32 g_debugchr_indicator_state[2];
extern u8 g_debugchr_primitive_tail_template[];
extern u8 g_debugchr_thread_indicator_packets[];

extern s32 debugchr_is_event_thread_running(s32 thread_id);
extern void submit_primitive(void* primitive);

void debugchr_render_thread_status_indicators(void) {
    s32* fade_work[1];
    ...
    active_thread_id = 0;
    if (debugchr_is_event_thread_running(8) != 0) {
    ...
    if (debugchr_is_event_thread_running(7) != 0) {
        event_thread = g_event_threads;
...
update_first_state:
    if (g_debugchr_indicator_state[0] == 1) {
        g_debugchr_indicator_state[0] = 2;
    }
    if (g_debugchr_indicator_state[1] != 1) {
        panel = 0;
        goto render_panels;
    }
    y = 2;
set_second_state:
    g_debugchr_indicator_state[1] = y;
    panel = 0;

render_panels:
    fade_work[0] = g_debugchr_indicator_fade_work;
    panel_offset = 0;
    brightness_ptr = (u8*)g_debugchr_indicator_brightness;
    packet_bank_offset = 0;
    do {
        packet_index = (s32*)((u8*)g_debugchr_indicator_packet_index + panel_offset);
        *packet_index ^= 1;
        packet = g_debugchr_thread_indicator_packets + packet_bank_offset + *packet_index * 0x13c;
        SetDrawMode(packet, 0, 0,
            GetTPage(0, 2, 0x3c0, 0x100) & 0xffff,
            (RECT*)g_debugchr_primitive_tail_template);
        tile = packet + 0xc;
        SetTile(tile);
    ...
    line_loop:
        ...
        indicator_state = *(s32*)((u8*)g_debugchr_indicator_state + panel_offset);
        if (indicator_state == 0) {
            ...
            if (indicator_state == 1) {
                ...
                __asm__ volatile(
                    "lui $8,%%hi(g_debugchr_indicator_brightness)\n"
                    "addiu $8,$8,%%lo(g_debugchr_indicator_brightness)\n"
                    "addu %0,%1,$8"
                    : "=r"(fade_in_word)
                    : "r"(panel_offset));
            ...
            state = (s32*)((u8*)g_debugchr_indicator_state + panel_offset);
            if (*state == 2) {
                s32* fade_out_word;
                /* Preserve the retail compiler's temporary address register. */
                __asm__ volatile(
