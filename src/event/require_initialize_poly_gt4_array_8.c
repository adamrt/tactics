...
extern POLY_GT4 g_require_poly_gt4_array_8[];
extern s32 GetGraphType(void);
extern u16 g_require_clut_origin_y;

void require_initialize_poly_gt4_array_8(s32 x, s32 y) {
    s32 count;
    s32 offset;
    s32 enabled;
    s16* texture_page;
    ...
    offset = 0;
    do {
        POLY_GT4* poly = (POLY_GT4*)((u8*)g_require_poly_gt4_array_8 + offset);
        SetPolyGT4(poly);
        SetShadeTex(poly, 0);
        SetSemiTrans(poly, 1);
        poly->u0 = 0;
