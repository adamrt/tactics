 ...
 * byte spelled out). */
typedef struct BunitMenuTile {
    u32 tag;
    u8 r, g, b, code;
    s16 x, y, w, h;
} BunitMenuTile;

typedef struct BunitMenuLineF2 {
    u32 tag;
    u8 r, g, b, code;
    s16 x0, y0, x1, y1;
} BunitMenuLineF2;
...
typedef struct BunitMenuPrimitives {
    DR_MODE draw_modes[3]; /* 0x000 */
    u8 _pad024[0x1e0];
    BunitMenuTile tiles[2];   /* 0x204 */
    BunitMenuLineF2 lines[8]; /* 0x224 */
} BunitMenuPrimitives;

/* The halfword stores go through volatile so they stay in source order. */
#define LINE(n) (((volatile BunitMenuLineF2*)menu->lines)[n])

void bunit_initialize_menu_tile_and_line_primitives(BunitMenuPrimitives* menu) {
    s32 i;
    s32 shared_value;
    s32 ten;
    ...
    register BunitMenuPrimitives* call_menu asm("$4");
