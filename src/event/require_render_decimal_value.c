...
typedef struct RequireTextPosition {
    u16 x;
    u16 y;
    u8 unknown_04[4];
    u16 row_stride;
...
extern RequireGlyph g_require_decimal_glyph;

extern void blit_text_glyph(void* text, void* pixels, RequireGlyph* glyph,
    RequireTextPosition* position);

void require_render_decimal_value(s32 value, s32 flags, void* pixels,
    RequireTextPosition* position) {
    s32 current_value = value;
    s32 render_flags = flags;
    void* render_pixels = pixels;
    RequireTextPosition* render_position = position;
    s32 saved_color;
    register s32 digit_index __asm__("$18");
    register RequireGlyph* glyph __asm__("$21");
    ...
    if ((render_flags & 0x8000) != 0) {
        render_position->x -= 2;
        g_require_decimal_glyph.source_x = 0xb4;
        blit_text_glyph(g_battle_text_pointers, render_pixels,
            &g_require_decimal_glyph, render_position);
        render_position->x += 7;
    }

    if ((render_flags & 0x0100) != 0) {
        g_require_decimal_glyph.source_x = 0x78;
        g_require_decimal_glyph.source_y = 0x1a;
        g_require_decimal_glyph.height = 4;
        render_position->y += 3;
        blit_text_glyph(g_battle_text_pointers, render_pixels,
            &g_require_decimal_glyph, render_position);
        render_position->x += 7;
        render_position->y -= 3;
        g_require_decimal_glyph.source_y = 0x10;
        g_require_decimal_glyph.height = 0x0a;
    }

    render_position->pixel_adjustment = saved_color;

    if ((render_flags & 0x0800) != 0) {
        g_require_decimal_glyph.source_x = 0xba;
        if (current_value == 0) {
    ...
    if (saved_color != 0) {
        g_require_decimal_glyph.source_x = 0xc8;
        blit_text_glyph(g_battle_text_pointers, render_pixels,
            &g_require_decimal_glyph, render_position);
        render_position->x += 5;
    }
