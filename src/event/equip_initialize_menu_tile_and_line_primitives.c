 ...
 * byte spelled out). */
typedef struct EquipMenuTile {
    u32 tag;
    u8 r, g, b, code;
    s16 x, y, w, h;
} EquipMenuTile;

typedef struct EquipMenuLineF2 {
    u32 tag;
    u8 r, g, b, code;
    s16 x0, y0, x1, y1;
} EquipMenuLineF2;
...
typedef struct EquipMenuPrimitives {
    DR_MODE draw_modes[3]; /* 0x000 */
    u8 _pad024[0x1e0];
    EquipMenuTile tiles[2];   /* 0x204 */
    EquipMenuLineF2 lines[8]; /* 0x224 */
} EquipMenuPrimitives;

/* The halfword stores go through volatile so they stay in source order. */
#define LINE(n) (((volatile EquipMenuLineF2*)menu->lines)[n])

void equip_initialize_menu_tile_and_line_primitives(EquipMenuPrimitives* menu) {
    s32 i;
    s32 shared_value;
    s32 ten;
    ...
    register EquipMenuPrimitives* call_menu asm("$4");
