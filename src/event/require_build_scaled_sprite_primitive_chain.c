...
typedef struct RequireScaledSprite {
    u32* ordering_tag;
    u8 intensity;
    u8 pad_05[3];
    s32 link_primitive;
...
extern u8 g_require_scaled_sprite_count;
extern s16 g_require_scaled_sprite_scale[];
extern s16 g_require_scaled_sprite_uv_rects[];
extern s16 g_require_primitive_tail_template[];
extern RequireTexturePage g_require_scaled_sprite_texture_page;

extern void require_set_scaled_poly_ft4_geometry_and_uv(
    u8* poly, const RequireTexturePage* texture, const s16* position,
    const s16* uv_rect, const s16* scale, const s16* offset);

void require_build_scaled_sprite_primitive_chain(
    RequireScaledSprite* sprite, u8* primitive_argument) {
    register u8* primitive __asm__("$16") = primitive_argument;
    s32 initial_count = g_require_scaled_sprite_count;
    s32 i = 0;
    ...
    if (initial_count > 0) {
        ...
        do {
            ...
            *(u16*)(primitive_fields + 8) = GetClut(0, 0x1fd);
            ...
            scaled_x = *(s16*)((u8*)g_require_scaled_sprite_scale + scale_offset);
            scaled_x *= sprite->scale_x;
            scaled_y = *(s16*)((u8*)g_require_scaled_sprite_scale + scale_offset + 2) * sprite->scale_y;
            ...
            require_set_scaled_poly_ft4_geometry_and_uv(
                primitive, &g_require_scaled_sprite_texture_page,
                g_require_primitive_tail_template,
                (s16*)((u8*)g_require_scaled_sprite_uv_rects + uv_offset),
            ...
            scale_offset += 4;
            i++;
        } while (i < g_require_scaled_sprite_count);
    }
