...
typedef struct BunitTextImagePosition {
    u16 x;
    s16 y;
    s16 row_stride;
} BunitTextImagePosition;

extern const u8* g_text_glyph_bitmap_data;
extern const u8 g_text_glyph_widths[];
extern void bunit_init_bit_reader(const void* data);
extern u32 bunit_read_bits(s32 bit_count);

s32 bunit_render_glyph_to_4bpp_image(
    s32 glyph_id, u8* image, const BunitTextImagePosition* position,
    s32 style) {
    s32 row;
    s32 palette_offset;
    ...
    bunit_init_bit_reader(glyph_bitmap);
