 ...
 * Same layout as {Attack,Bunit,Require}StatusPanelPrimitives.
 *
 ...
 * matching cookbook. */
typedef struct DebugChrStatusPanelPrimitives {
    u8 header[0x10];
    SPRT sprites[25]; /* 0x010 */
} DebugChrStatusPanelPrimitives;

/* Same shape as AttackFrameConfig in attack_configure_status_panel_primitive_colors.c. */
typedef struct DebugChrFrameConfig {
    u8 reserved_00[0x10];
    s32 style;
} DebugChrFrameConfig;
...
void debugchr_configure_status_panel_primitive_colors(DebugChrStatusPanelPrimitives* primitives,
    const DebugChrFrameConfig* state) {
    s32 i;
    s32 clut;
