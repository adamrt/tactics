 ...
 * byte spelled out). */
typedef struct RequireMenuTile {
    u32 tag;
    u8 r, g, b, code;
    s16 x, y, w, h;
} RequireMenuTile;

typedef struct RequireMenuLineF2 {
    u32 tag;
    u8 r, g, b, code;
    s16 x0, y0, x1, y1;
} RequireMenuLineF2;
...
typedef struct RequireMenuPrimitives {
    DR_MODE draw_modes[3]; /* 0x000 */
    u8 _pad024[0x1e0];
    RequireMenuTile tiles[2];   /* 0x204 */
    RequireMenuLineF2 lines[8]; /* 0x224 */
} RequireMenuPrimitives;

/* The halfword stores go through volatile so they stay in source order. */
#define LINE(n) (((volatile RequireMenuLineF2*)menu->lines)[n])

void require_initialize_postbattle_unit_menu_primitives(RequireMenuPrimitives* menu) {
    s32 i;
    s32 shared_value;
    s32 ten;
    ...
    register RequireMenuPrimitives* call_menu asm("$4");
