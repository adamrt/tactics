...
typedef struct BunitTexturedQuadDescriptor {
    u8 x;
    u8 y;
    u8 w;
    u8 h;
    u8 u;
    u8 v;
} BunitTexturedQuadDescriptor;

extern u16 g_bunit_poly_ft4_count;
extern u8* g_bunit_graphics_context;

void bunit_enqueue_textured_quad_list(const BunitTexturedQuadDescriptor* quads,
    ...
    do {
        POLY_FT4* poly;
        u16 index = g_bunit_poly_ft4_count;

        g_bunit_poly_ft4_count = index + 1;
        poly = (POLY_FT4*)(*(u8**)(g_bunit_graphics_context + 0x10) + index * sizeof(POLY_FT4));
        ...
        {
            u8* context = g_bunit_graphics_context;
