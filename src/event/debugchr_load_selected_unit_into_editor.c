...
extern s16 g_debugchr_comparison_unit_id;
extern s16 g_debugchr_selected_unit_id;
extern u8 g_debugchr_comparison_billboard[];
extern u8 g_debugchr_selected_billboard[];
extern s16 g_debugchr_unit_type;
extern s16 g_debugchr_brave;
extern s16 g_debugchr_faith;
extern s16 g_debugchr_zodiac;
extern s16 g_debugchr_editor_unit_fields[];
extern u8 g_debugchr_first_ability_bytes[];
extern u8 g_debugchr_second_ability_bytes[];
extern u8 g_debugchr_third_ability_bytes[];
extern u8 g_debugchr_fourth_ability_bytes[];
...
extern void debugchr_copy_unit_data_to_status_billboard(battle_stats_t* unit, u8* output);
...
void debugchr_load_selected_unit_into_editor(void) {
    ...
    unit = get_battle_stats_pointer(g_debugchr_comparison_unit_id);
    editor = g_debugchr_editor_unit_fields;
    if (unit != 0) {
        debugchr_copy_unit_data_to_status_billboard(
            unit, g_debugchr_comparison_billboard);
    }

    unit = get_battle_stats_pointer(g_debugchr_selected_unit_id);
    if (unit == 0)
        return;
    debugchr_copy_unit_data_to_status_billboard(
        unit, g_debugchr_selected_billboard);

    g_debugchr_unit_type = unit->job_id;
    g_debugchr_brave = unit->brave;
    g_debugchr_faith = unit->faith;
    /* The zodiac sign lives in the top nibble of the halfword at 0x008. */
    g_debugchr_zodiac = *(u16*)&unit->_pad008 >> 12;
    ...
    if (editor[7] != 0) {
        ...
        if (field_value == 0)
            g_debugchr_third_ability_bytes[0] = 0xfe;
        field_value = unit->secondary_skillset;
        editor[24] = field_value;
        if (field_value == 0)
            g_debugchr_fourth_ability_bytes[1] = 0xfe;
    ...
    debugchr_fill_16_bytes_with_fe(g_debugchr_first_ability_bytes);
    debugchr_fill_16_bytes_with_fe(g_debugchr_second_ability_bytes);
    debugchr_fill_16_bytes_with_fe(g_debugchr_third_ability_bytes);
    debugchr_fill_16_bytes_with_fe(g_debugchr_fourth_ability_bytes);
    ...
    world_copy_bytes(g_debugchr_first_ability_bytes, &unit->learned_abilities[0x12c - 0x99], 0x10);
    world_copy_bytes(g_debugchr_second_ability_bytes, &unit->learned_abilities[0x13c - 0x99], 0x10);
    world_copy_bytes(g_debugchr_third_ability_bytes, &unit->learned_abilities[0x14c - 0x99], 0x10);
    world_copy_bytes(g_debugchr_fourth_ability_bytes, &unit->learned_abilities[0x154 - 0x99], 0x10);
}
