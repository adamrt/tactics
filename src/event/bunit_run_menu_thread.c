...
extern s32* g_bunit_controller_input;
...
void bunit_run_menu_thread(void) {
    u8 local_10[8];
    u8 frames[0xf8];
    s32 saved;
    s32 frame;
    ...
    s32 window_id;
    /* Pins + identity fences: the constant 1 for the a3 argument must be
     * materialised before `one = 1` ($s4); hoisting an explicit call_one
     * local, moving `one = 1`, and `*refresh = one` all leave GCC emitting
     * `li s4,1` first. The frame_buffer fence fixes `move s0,v0`. */
    ...
    g_bunit_controller_input = battle_get_controller_input_pointer(0);
    ...
    *refresh = 1;
    for (;;) {
        ...
        controller_input = g_bunit_controller_input;
