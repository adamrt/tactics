...
typedef struct JobsttsTextImagePosition {
    u16 x;
    s16 y;
    s16 row_stride;
} JobsttsTextImagePosition;

extern const u8* g_text_glyph_bitmap_data;
extern const u8 g_text_glyph_widths[];
extern void jobstts_set_primary_bitstream_source(const void* data);
extern u32 jobstts_read_primary_bits(s32 bit_count);

s32 jobstts_render_glyph_to_4bpp_image(
    s32 glyph_id, u8* image, const JobsttsTextImagePosition* position,
    s32 style) {
    s32 row;
    s32 palette_offset;
    ...
    jobstts_set_primary_bitstream_source(glyph_bitmap);
    ...
    do {
        ...
        do {
            ...
            pixel = jobstts_read_primary_bits(2);
