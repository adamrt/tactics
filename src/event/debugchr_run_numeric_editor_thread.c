...
extern void debugchr_initialize_portrait_primitive_tail(void* primitive);
...
extern void debugchr_render_signed_decimal_entries(s32 base, void* entries,
...
extern void debugchr_configure_scaled_draw_area_packets(
    void* packets, const void* geometry, s32 buffer, s32 frame_offset,
    void* thread);
extern void SetDrawOffset(void* destination, void* position);
extern void debugchr_initialize_menu_primitive_colors(void* primitives,
    void* thread);
...
extern s16 g_debugchr_draw_area_x;

extern u8 g_debugchr_numeric_state_a[];
extern u8 g_debugchr_numeric_state_b[];
extern u8 g_debugchr_numeric_text_a[];
extern u8 g_debugchr_numeric_text_b[];
extern u8 g_debugchr_numeric_entries_a[];
extern u8 g_debugchr_numeric_entries_b[];
extern u8 g_debugchr_numeric_entries_c[];
extern u8 g_debugchr_numeric_entries_d[];
extern u8 g_debugchr_numeric_entries_e[];
extern u8 g_debugchr_numeric_entries_f[];
extern u8 g_debugchr_numeric_descriptor_a[];
extern u8 g_debugchr_numeric_descriptor_b[];
extern u8 g_debugchr_numeric_table[];
extern u8 g_debugchr_numeric_geometry[];
extern u8 g_debugchr_numeric_texture[];

void debugchr_run_numeric_editor_thread(void) {
...
#define upload_c   (*(u8* volatile*)&stack_pointers[3].value)
    ...
    g_debugchr_controller_input = battle_get_controller_input_pointer(0) + 1;

    if (g_current_event_thread_id == 0xc) {
        state = g_debugchr_numeric_state_a;
        text = g_debugchr_numeric_text_a;
        selected_upload_c = g_debugchr_numeric_entries_a;
        upload_a = selected_upload_c;
        selected_upload_c = g_debugchr_numeric_entries_b;
        upload_b = selected_upload_c;
        selected_upload_c = g_debugchr_numeric_entries_c;
        entries = g_debugchr_numeric_descriptor_a;
        *(volatile s32*)&stack_pointers[0] = (s32)state;
        goto numeric_buffers_selected;
    }
    state = g_debugchr_numeric_state_b;
    text = g_debugchr_numeric_text_b;
    selected_upload_c = g_debugchr_numeric_entries_d;
    upload_a = selected_upload_c;
    selected_upload_c = g_debugchr_numeric_entries_e;
    upload_b = selected_upload_c;
    selected_upload_c = g_debugchr_numeric_entries_f;
    entries = g_debugchr_numeric_descriptor_b;
    state_base = g_debugchr_numeric_state_b;
numeric_buffers_selected:
    ...
    battle_initialize_numeric_display_frame_primitives(g_debugchr_numeric_table,
        state + 0x18c);
    debugchr_initialize_portrait_primitive_tail(state + 0x298);

    i = 0;
    {
        u8* descriptor = g_debugchr_numeric_texture;
        u8* primitive;
        frame = 0x24;
        do {
            primitive = state + frame;
            battle_initialize_semitransparent_menu_sprt(primitive);
            configure_textured_primitive_geometry(primitive, g_debugchr_numeric_geometry,
                g_debugchr_numeric_table, descriptor);
            descriptor += 0xc;
            i++;
            frame += 0x14;
        } while (i < 0x12);
    ...
    frame = 0;
    {
        ...
        for (;;) {
            ...
            if (frame == 0 || S32_AT(thread, 0xc) != 0) {
                ...
                *origin = 0x14;
                ...
                if (g_current_event_thread_id != twelve)
                    debugchr_render_signed_decimal_entries((s32)text, entries,
                        (u8*)origin - 8, 3);
                else
                ...
                *origin = 0x40;
                world_set_menu_text_origin(0, 0);
                if (g_current_event_thread_id != twelve)
                    debugchr_render_signed_decimal_entries((s32)(text + 0x168),
                ...
                *origin = 0x64;
                world_set_menu_text_origin(0, 0);
                if (g_current_event_thread_id != twelve)
                    debugchr_render_signed_decimal_entries((s32)(text + 0x468),
...

            {
                ...
                different = -different;
                g_debugchr_draw_area_x = different & 0xf0;
                debugchr_configure_scaled_draw_area_packets(buffer + 0x298,
                    g_debugchr_numeric_table + 8, frame, scaled, thread);
            }
            {
                u8* primitive = buffer + 0x278;
                register s32 x asm("$2") = U16_AT(thread, 0);
                s32 draw_area = (u16)g_debugchr_draw_area_x;
                x -= 0x80;
            ...
            debugchr_initialize_menu_primitive_colors(buffer + 0x18c, thread);
