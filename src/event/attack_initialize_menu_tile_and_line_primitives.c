...
/* libgpu TILE / LINE_F2 packets with the code byte spelled out. */
typedef struct AttackMenuTile {
    u32 tag;
    u8 r, g, b, code;
    s16 x, y, w, h;
} AttackMenuTile;

typedef struct AttackMenuLineF2 {
    u32 tag;
    u8 r, g, b, code;
    s16 x0, y0, x1, y1;
} AttackMenuLineF2;
...
typedef struct AttackMenuPrimitives {
    DR_MODE draw_modes[3]; /* 0x000 */
    u8 _pad024[0x1e0];
    AttackMenuTile tiles[2];   /* 0x204 */
    AttackMenuLineF2 lines[8]; /* 0x224 */
} AttackMenuPrimitives;

/* The halfword stores go through volatile so they stay in source order. */
#define LINE(n) (((volatile AttackMenuLineF2*)menu->lines)[n])

void attack_initialize_menu_tile_and_line_primitives(AttackMenuPrimitives* menu) {
    s32 i;
    s32 shared_value;
    s32 ten;
    ...
    register AttackMenuPrimitives* call_menu asm("$4");
