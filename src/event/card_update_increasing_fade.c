...
s32 card_update_increasing_fade(void) {
    volatile u32 stack_padding[4];
    s32 result = 1;
    s32 speed = card_get_event_speed();

    if (g_card_increasing_fade_active != 0) {
        g_card_fade_intensity += speed * 8;
        if (g_card_fade_intensity >= 0x100) {
            result = 0;
            g_card_increasing_fade_active = 0;
            g_card_fade_intensity = 0xff;
        }
