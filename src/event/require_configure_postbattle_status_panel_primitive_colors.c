 ...
 * Same layout as {Attack,Bunit,DebugChr}StatusPanelPrimitives.
 *
 ...
 * matching cookbook. */
typedef struct RequireStatusPanelPrimitives {
    u8 header[0x10];
    SPRT sprites[25]; /* 0x010 */
} RequireStatusPanelPrimitives;

/* Same shape as AttackFrameConfig in attack_configure_status_panel_primitive_colors.c. */
typedef struct RequireFrameConfig {
    u8 reserved_00[0x10];
    s32 style;
} RequireFrameConfig;
...
void require_configure_postbattle_status_panel_primitive_colors(RequireStatusPanelPrimitives* primitives,
    const RequireFrameConfig* state) {
    s32 i;
    s32 clut;
