...
extern s32 require_collect_low_brave_high_faith_departures(void);
...
extern void* get_battle_stats_pointer(s32 index);
...
void require_remove_units_with_low_brave_or_high_faith(void) {
    s32 i;
    s32 main_thread;

    if (require_collect_low_brave_high_faith_departures() == 0)
    ...
    main_thread = battle_get_next_available_thread_id(0x10);
    initialize_event_thread(main_thread, require_render_display_condition_special_cases_thread);
    battle_store_thread_function_parameters(main_thread, 6, 0, 0);
    require_fade_rgb_31_frames(0xd, 5, 2);
    switch_to_next_event_thread();
    for (i = 0; i < g_require_affected_unit_count; i++) {
        ...
        unit = get_battle_stats_pointer(g_require_affected_battle_unit_indices[i]);
        message_thread = battle_get_next_available_thread_id(0x10);
        initialize_event_thread(message_thread, require_render_display_condition_special_cases_thread);
        if (unit[6] & 0x40) {
            battle_store_thread_function_parameters(message_thread, 9, 1, 0);
        } else {
            battle_store_thread_function_parameters(message_thread, 8, 1, 0);
        }
        wait_for_event_thread(unit_thread);
        battle_store_thread_function_parameters(message_thread, 0, 0, 2);
        wait_for_event_thread(message_thread);
        wait_frames(0x1e);
        main_remove_unit_from_party(unit[2]);
    }
    battle_store_thread_function_parameters(main_thread, 6, 0, 2);
