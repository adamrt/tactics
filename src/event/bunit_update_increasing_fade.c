...
s32 bunit_update_increasing_fade(void) {
    u8 unused[16];
    s32 ret = 1;
    s32 speed = bunit_get_event_speed();

    if (g_bunit_increasing_fade_active != 0) {
        g_bunit_fade_intensity += speed * 8;
        if (g_bunit_fade_intensity >= 0x100) {
            g_bunit_increasing_fade_active = 0;
            g_bunit_fade_intensity = 0xFF;
            ret = 0;
        }
